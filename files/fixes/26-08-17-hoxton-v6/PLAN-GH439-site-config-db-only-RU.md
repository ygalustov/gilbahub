...
семи контроллерах (восемь точек вызова) через одну функцию — а колонку читает только штамп, который пятьдесят третье и так убирает. Значит
...
писать исполнителя и трогать семь контроллеров ради симметрии. Симметрия не факт клиента; остаётся как известная
...
- `effectiveMethodology` (`Controller.php:23-28`) — базовая для семи контроллеров (восемь точек вызова) и держит сразу **вывод по региону**
  ...
  пустоте возвращает `null` — исход `empty`, который семь контроллеров обязаны показать («methodology not set»), а не
