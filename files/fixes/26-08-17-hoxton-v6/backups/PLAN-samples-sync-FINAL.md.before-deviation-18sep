...
## У ПЛАНА ЕСТЬ ДАТА, И ОН НЕ УЧИТЫВАЕТ ПРИНЯТОГО ПОСЛЕ НЕЁ

Записано 18.09.2026 по поводу, найденному при исполнении стадии 2. **Не ошибка автора, а
свойство всякого плана.**

**Повод.** Пункт плана велел убрать `withTrashed()` ЦЕЛИКОМ. Буквальное исполнение отменило бы
**GH-431** — работу, принятую на этой же неделе: три утверждения про `skipped` стали бы
красными, а устаревшая вкладка снова воскрешала бы пробу новой строкой. Причина простая и
неприятная: **план написан ДО GH-431.**

Заметить это можно было только замером, а не чтением плана. Исполнитель заметил и сделал
третий режим — только для `store()`, — вместо буквального исполнения.

**Правило, которое из этого следует.** Прежде чем исполнять пункт плана буквально, проверяется,
не отменяет ли он что-то принятое ПОЗЖЕ даты плана.

**Чем проверять, дешевле всего:** по changelog — что появилось между датой плана и сегодня.
Этот план написан 10.09.2026.

**Три остальных отклонения той же сдачи, того же рода:** функции `syncSiteListToServer()`, в
которую план направляет три события, не существует с GH-441; `client_uid` в теле PATCH не
шлётся, потому что сервер его перезаписывает, а это ключ повторного импорта; `siteId` добавлен
в девять диспатчей, а не в четыре, по требованию самой поправки.
...
6. **Загрузка — только с сервера** (стадия 3): `restoreFromLocalFallback()` и все `_ls.setItem` (`:166, 471, 545, 715`)
   ...
   **ПЕРЕМЕРЕНО 18.09.2026, и поле названо явно, потому что при пересказе число один раз приклеилось к другому.**
   Всего строк **148** (147 плюс проба-зонд живого прогона, оставленная мягко удалённой).
   Без `client_uid` — **27**. Без ярлыка `payload._label` — **9**. Число 27 относится к
   **`client_uid`**, не к ярлыку. Адресом не годится ни то, ни другое, но по разным причинам и в
   разных количествах.
...
8. **D-3 — мягкое удаление логов при `clearSiteData`** (пользователь принял; реализация **не** «SoftDeletes на
   ...
   **РЕШЕНО ВЛАДЕЛЬЦЕМ 18.09.2026. Вопрос закрыт, менять нечего.** Её слова дословно:

   «Когда мы делаем импорт сайта, у нас стирается полностью всё, что было раньше, и всё загружается из этого
   файла. Это не просто пробы импортировать, а это импорт сайта. Он как раз сделан для того, чтобы удалить всё
   существующее и загрузить всё по этому сайту из файла. Если там чего-то нет, it's ok. Но идея в том, что
   полностью удаляем всё, что было, и заново загружаем всё из файла.»

   То есть нынешнее поведение ВЕРНО: журналы стираются потому, что стирается вся площадка, и отсутствие журналов
   в файле — не потеря, а следствие замысла. Прежняя формулировка этого пункта («открытый вопрос: бандл импорта
   не содержит логов, то есть импорт удаляет логи, которые ничем не заменяет») снята.

   **ОТМЕНЕНО ТЕМ ЖЕ ДНЁМ, 18.09.2026: D-3 стадии 1 откачен владельцем.** Её слова: «давай жёстко только
   журнал удалять, оставим как было с пробами». То есть журналы при импорте снова удаляются ЖЁСТКО, как было
   до 18.09; пробы не трогаются — их мягкое удаление стоит с 29 апреля и защищает лабораторные данные.

   Откат безопасен: мягко удалённых строк в журналах нет ни одной (`spray_logs` 3 живых / 0 удалённых,
   `field_log_entries` пусто). В откат входит снятие шестнадцати условий `whereNull` и удаление колонок
   `deleted_at` с обеих таблиц: механизм, которым никто не пользуется, читается как действующий.

   **Абзац ниже описывает состояние, которое просуществовало один день, и оставлен как след:** стадия 1 сделала
   это удаление МЯГКИМ. Для пользователя поведение то же — после импорта прежнего нет, — но строки не уничтожаются, а
   прячутся, и их можно вернуть. Решение владельца это не нарушает: она описала, что должно ПРОИСХОДИТЬ, а
   мягкость — про то, как это устроено внутри. Наблюдаемого это не меняет, поэтому отдельно не спрашивалось.
