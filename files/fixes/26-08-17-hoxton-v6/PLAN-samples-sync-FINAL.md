...
## СВЕРКА ПЛАНА С ДЕРЕВОМ, 18.09.2026

**Читать до плана.** Документ написан 10.09.2026 и с тех пор одна стадия закрыта, а текст
по-прежнему описывает её как предстоящую. Сверено чтением кода и маршрутов, живых прогонов
не было.

- **Стадия 0 — СДЕЛАНА (GH-430, GH-431).** `reconcileMissingSnapshotSamples()` в дереве нет;
  на её месте комментарий, `SampleController.php:195`. Везде ниже, где стадия 0 описана как
  предстоящая, читать как сделанное.
- **Стадия 1 — НЕ СДЕЛАНА.** Маршрута `DELETE /api/samples/{id}` в `routes/web.php` нет:
  объявлены `index`, `store`, `sync`, `show`, `update`, `analyse` (`:108-113`). Удаление
  проб по-прежнему идёт только через снимок. `DataController::destroy` на месте (`:109`),
  а стадия 1 велела его убрать.
- **Стадия 2 — НЕ СДЕЛАНА.** Сервер по-прежнему воскрешает: `$sample->restore()` в
  `SampleController.php:459`. Браузер по-прежнему шлёт коллекцию целиком —
  `sample-persistence.js:681-692`, шесть страниц, смена площадки и ещё десять событий.
- **Стадии 3 и 4 — НЕ СДЕЛАНЫ.** Браузерная копия на месте.

**Что из этого следует для плана:** направление и стадии 1–4 в силе, отменять нечего.
Устарела ровно одна вещь — очерёдность: первой теперь идёт стадия 1, а не стадия 0.

**Порядок работ, решение владельца 18.09.2026:** пробы — следующая работа после трёх
нынешних заданий аналитику. Её слова: «насчёт проб у нас где-то был один план, что с ними
сделать; сейчас текущие какие-то вопросы закроем и перейдём к пробам».

---

## УСТРОЙСТВО: КАК ИЗМЕНЕНИЕ В ПРОБАХ ДОХОДИТ ДО СЕРВЕРА — 18.09.2026, Аня (задание Евы, согласовано владельцем)

Задание: «страница шлёт состояние, а не изменение; правило владельца — база единственный источник правды, браузер
шлёт изменение». Только план. Заявки задания и сверка Ники проверены по дереву и по базе (замеры сняты мной 18.09;
данные стенда двигались в 08:58 — сохранённый план Russley, на пробы не влияет). Направление документа (раздел 2) и
стадии 1–4 — в силе; ниже — устройство самой записи, поправки к стадиям там, где дерево или решения 17–18.09 их
изменили, и порядок.

### A. Сверка: что подтверждается, что расходится ещё

Подтверждено по дереву: стадия 0 сделана (`reconcileMissingSnapshotSamples()` нет; GH-430); маршрута
`DELETE /api/samples/{id}` нет (`app/routes/web.php:108-113` — `index, store, sync, show, update, analyse`), удаление —
только `DELETE /api/data/entry/{id}` (`:125`, `DataController::destroy` `:109`); браузер шлёт коллекцию целиком —
`doSave()` берёт `getAllSamples()` и отдаёт `syncSamplesToServer(snapshot)` → `POST samples/sync {allSites}`
(`assets/sample-persistence.js:264-311, 340-352`); триггеры — 11 событий `MUTATION_EVENTS` (`:681-692`): 10 мутаций +
`gaip:site-changed`; страницы — `analysis`, `field-log`, `hub`, `reports/{export,scenarios,forensic}` (шесть, как в
задании); второй кэш `gilba_hub_samples` в `hub-persistence.js` на месте (`:237, 887-898, 2167-2169, 2280-2317, 2348, 2378`);
штамп `dashboard-ui.js:74-88` пишет `gilba_samples` при Re-run.
База 18.09: `samples` — 147 строк, 87 trashed, 27 без `client_uid`; живых 60 (Burns 26 soil / 2 tissue / 1 water; New
test 13 soil / 5 water; Russley 3/1/2; Federal Golf, test4, Test5 (+1 tissue), Westview, оба Test - GC — по одной).
Колонок `deleted_by_user_id`/`delete_source` нет — стадия 1 не начиналась.

Расходится со сверкой ещё в одном месте, и это меняет стадию 2: **воскрешение снято наполовину.** GH-431 дал `sync()`
allow-list (`$restorableTrashedIds`, `SampleController.php:179, 284, 455-456`): при обычном push список пуст, и
`saveSampleRecord()` мягко удалённую строку **не** восстанавливает, а считает `skipped`. Но `store()` (`:66-114`) зовёт
`saveSampleRecord()` **без** allow-list (`null` = «восстанавливать можно», `:445`), и `POST /api/samples` с совпадающим
`(site, type, client_uid)` **воскрешает** trashed-строку. Сверка пишет «сервер по-прежнему воскрешает» без этого различия;
для устройства оно важно: путь, на который переезжает запись (POST), — как раз тот, где воскрешение осталось.

### B. Устройство — изменение, не состояние

Одно правило на всё: **запрос на сервер — это намерение об одной записи, порождённое действием пользователя в момент
действия**; в теле — только эта запись; стор в памяти — читающий кэш (правило проекта это допускает), никогда не
источник для записи; `localStorage` — ни источник, ни адресат.

1. **Единица намерения — запись, адрес — серверный `id`.** Объект пробы в сторе получает `serverId` — `id` строки из
   ответа `GET /api/samples` (`samplePayload()` отдаёт `id`, `SampleController.php:521`) и из `201` на `POST`. Сегодня
   стор его не держит нигде (`serverId` в `sample-persistence.js`/`sample-manager.js` — ноль вхождений; объект строится
   как `{id: client_uid || label…, values: payload, …}`, `:396-452`). `client_uid` остаётся ярлыком и ключом повторного
   импорта (D-7), **адресом записи он быть не может**: у 27 из 147 строк его нет (Data page шлёт `null`), а ярлычные uid
   («Green 1») повторяемы между площадками и вкладками. Маршруты по-записные уже есть для `PATCH`/`GET` по `{sample}`
   (route-model-binding по числовому `id`); `DELETE /api/samples/{sample}` — стадия 1.
2. **Событие → один запрос, по таблице стадии 2 п. 4**, с четырьмя поправками к ней:
   - **площадка — из записи, не из указателя.** Таблица шлёт `site_id: SM.getActiveSiteId()` — это указатель страницы,
     класс GH-459 (сорок пятое, пятьдесят пятое в PLAN-GH439): проба, добавленная за миг до переключения, уйдёт на другую
     площадку. Проба несёт `siteId` с момента `addSample` (стор и так кладёт её в `allSites[siteId]` — тот ключ и
     есть её площадка), событие несёт его в `detail`, запрос берёт из `detail`, не из `getActiveSiteId()`;
   - **события несут то, что нужно запросу.** Сегодня `detail` — `{dataType, sampleId, sample}` у add/update
     (`sample-manager.js:1898, 2097, 2363`), `{dataType, sampleId}` у delete (`:2117`), `{dataType}` у `samples-cleared`
     (`:2164`), ничего у `all-samples-cleared` (`:2182`); `gaip:sample-turf-profile-changed` (`:2400`) в `MUTATION_EVENTS`
     не входит. Добавляется `siteId` везде, `serverIds` — в оба clear-события (снимаются **до** очистки стора);
     `turf-profile-changed` — в список; `gaip:site-changed` — из списка (переключение — не изменение данных);
   - **`gaip:site-added/renamed/removed` → `POST /api/sites/sync`** — это тоже снимок (реестра площадок), и PLAN-GH439
     (стадия 2, риск (г)) его из `sample-persistence.js` уже убирает: реестр строится только из `GET /api/sites`, а
     добавление/переименование площадки идёт по-записными маршрутами сайтов там, где оно делается (Settings, мастер).
     В этом устройстве `sample-persistence.js` о площадках не пишет ничего;
   - **порядок по записи**: операции над одной записью — цепочка обещаний: `PATCH`/`DELETE` ждут `201` от `POST`
     (иначе нет `serverId`); операции над разными записями независимы. Дебаунс 500 мс (`scheduleSave`) снимается — он
     склеивал события в один снимок; по-записному запросу склеивать нечего.
3. **Сервер принимает намерение, не картину.** Стадия 1 — `destroy()` (одна точка удаления; `DataController::destroy`
   и `/data/entry/{id}` уходят); завершение стадии 2 на сервере — `store()` зовёт `saveSampleRecord()` **с пустым
   allow-list**, как `sync()`: совпадение с trashed-строкой — новая живая строка, не восстановление (GH-431 сделал это для
   `sync`, для `store` — нет). `sync` остаётся **только** у Settings-импорта с `clearSiteData: true` и одной площадкой
   (`settings-init.js:1789`) — это явное намерение «заменить всё содержимое площадки файлом», бульк-намерение, а не
   снимок состояния страницы; сужение (одна площадка, 422 иначе) — стадия 1 п. 5.
4. **Штампы на пробе — из настроек.** `saveSampleRecord()` пишет `methodology_snapshot` из `config.turf.methodology`
   (план по методологии, сдача A п. 3; решение владельца 18.09), `soil_texture_snapshot` — как сейчас из колонки; при
   повторном сохранении той же записи (`PATCH`) штампы **не переписываются** — они о моменте создания; сегодня
   `saveSampleRecord` перезаписывает их при каждом `firstOrNew`-совпадении (`:467-484`) — после перехода на `PATCH` по
   `serverId` путь через `firstOrNew` для существующих проб не используется, и вопрос снимается; для `sync` (импорт с
   очисткой) строки новые — штамп ставится.
5. **Отказ виден, молчания нет** (двадцать восьмое, пятидесятое в PLAN-GH439). Ответ ≠ 2xx или сеть → у записи исход
   `{op, status, at}`, стор не блокируется, запись не исчезает; в переключателе проб — маркер и Retry по записи;
   403 — «You don't have permission to edit this site», 419 — «Session expired — reload». Права: `canEditActiveSite` в
   `GAIP_HUB_CONFIG`, при `false` — ни одного запроса записи и скрытые кнопки (стадия 2 п. 6). **Не `skip` и не
   `warn`** — сегодня 403 уходит в `warn` (раздел 1, факт 16).
6. **Загрузка — только с сервера** (стадия 3): `restoreFromLocalFallback()` и все `_ls.setItem` (`:166, 471, 545, 715`)
   уходят, второй кэш `hub-persistence.js` (`keys.samples`, `collectSamples/restoreSamples`, `scheduleSave` по
   input/change `:2075-2127`) уходит, штамп `dashboard-ui.js:74-88` уходит; «не удалось загрузить» отличается от «проб
   нет» по разделу 5 (баннер, блокировка записи, Generate disabled). Исход загрузки — три состояния двадцать восьмого:
   `loaded` / `empty` / `unavailable` с причиной; `_gaipSamplePersistenceReady` ставится во всех трёх.

### B1. Приёмка 18.09.2026

Устройство принято по направлению владельцем («идея с идентификаторами мне нравится, чтобы нам полностью рабочую
систему сделать с сохранением баз данных»). Находка о воскрешении через `store()` (`SampleController.php:86-102` против
`:445-460`; `sync` передаёт список на `:284`) подтверждена Никой по адресам и передана владельцу. **Условие сдачи стадии
2, требование координатора:** воскрешение в `store()` снимается **в той же сдаче**, что переезд записи на этот путь —
раздельно нельзя: поштучная запись переедет ровно на путь, где воскрешение осталось, и дефект приедет вместе с ней.
Очередь работ (владелец, 18.09): живые тесты → методология A → методология B → пробы 1 → 2 → 3 (→ 4) → подстановки.

### C. Что меняется в стадиях документа

- **Порядок:** 1 → 2 → 3 (→ 4). Стадия 0 закрыта; между 1 и 2 не растягивать (факты 11, 17). Стадия 2 — один коммит:
  `serverId` + по-записные обработчики + `store()` без воскрешения + `rawData = payload` (факт 11: атомарно с удалением
  snapshot-push).
- **Стадия 2 п. 4 (таблица)** — четыре поправки из B.2: площадка из записи; `detail` с `siteId`/`serverIds`;
  `turf-profile-changed` в список; строки `site-added/renamed/removed` из таблицы **убрать** (реестр площадок — не
  забота `sample-persistence.js`).
- **Стадия 2 п. 1 (сервер)** — не «убрать `withTrashed()` везде», а **сделать `store()` равным `sync()`**: allow-list пустой;
  `withTrashed()->firstOrNew` может остаться для `sync` с `clearSiteData` (там `null` законен — строки только что
  удалены этим же запросом). Тест `test_…_can_restore_soft_deleted_sample` переписывается на «не воскрешает» для обоих
  путей.
- **Стадия 1 п. 6 (`index` cap 2000 + `meta`)** — в силе; сегодня `max:200` (`:23`), Burns один держит 29, New test 18.
- **Раздел 5 (состояние ошибки)** — в силе; дополняется исходом загрузки по двадцать восьмому и тем, что «пусто» при
  `unavailable` **не** показывается как пустой список.
- **Что не в этой работе, названо:** конфиги площадок (`site-config-persistence.js`) — GH-439; `sites/sync` из
  `sample-persistence.js` — уходит здесь, потому что это тот же снимок, а его замена (по-записные маршруты сайтов) уже
  есть; ролевая модель на сервере для новых маршрутов — стадия 1 (правило проекта).

### D. Чем проверяется — от того, что ушло на сервер и что легло в базу

- **Журнал запросов страницы** (сорок девятое): за сессию с N действиями пользователя над пробами — ровно N запросов к
  `/api/samples*`, каждый с одним `serverId`/одной записью, ни один с `allSites`; `gaip:site-changed` — ноль запросов к
  `/samples`. Страж: тело ни одного запроса из `sample-persistence.js` не содержит `allSites` (стадия 2, Jest); мутация
  ревьюера — вернуть `syncSamplesToServer(snapshot)` в любой обработчик → красный.
- **Четыре точки по базе** (`samples`, `site_summaries`) до и после каждого действия на живом стенде: add → +1 строка с
  `site_id` = площадка записи (а не активная, если их развести), нет trashed; update → тот же `id`, `payload` целиком
  (фикстура с ключом `thatch`, факт 17), штампы не изменились; delete → `deleted_at`, `delete_source`; clear → N
  `deleted_at`; повторный `POST` с тем же `client_uid` после удаления → новая строка, старая trashed (не воскрешена).
- **Площадка из записи, не из указателя:** добавить пробу, переключить площадку до ответа (`route.delay`), проверить
  `site_id` в базе — площадка записи; сегодняшний `getActiveSiteId()` в таблице стадии 2 дал бы вторую.
- **Загрузка**: перехват `/api/samples` → `unavailable`, баннер, ноль записей, Generate disabled; `[]` → `empty`, стор
  разблокирован; обычная загрузка — стор равен ответу API до записи (равенство форм в обе стороны, `store-shapes`).
- **Два кэша ушли**: после стадии 3 `localStorage` не содержит `gilba_samples`/`gilba_hub_samples*` после любого
  действия (проверка по значению в живом прогоне, не grep по коду); `DELETE /api/data/entry/{id}` не существует.
- **Живой набор** — по разделу 10 (пункты 1–8), с поправкой (3): delete через `DELETE /api/samples/{id}`.

### E. Подводные камни, кроме уже названных в разделах 7–8

- **Воскрешение через `store()`** — до завершения стадии 2 любой по-записный `POST` с ярлычным uid возвращает удалённую
  строку; поэтому по-записный клиент **не** включается раньше серверной половины стадии 2 (один коммит).
- **`client_uid = null`** у 27 строк: `PATCH`/`DELETE` по `serverId` их видят; `POST` для них идемпотентен только по
  `serverId` в ответе — повтор `POST` при обрыве сети даст дубликат; клиент помечает запись `pending` до `201` и не
  повторяет `POST` сам, пока не получит ответ или явный Retry.
- **`gaip:sample-updated` шлётся и миграцией** (`:1756`, `reason: 'b35fix411-migration'`) — не действие пользователя;
  обработчик пропускает события с `reason`, не `PATCH`-ит их (иначе миграция при загрузке даст N запросов).
- **Штамп методологии** зависит от плана по методологии (сдача A); порядок между двумя планами: штамп из конфига можно
  ставить и до остального — это одна строка в `saveSampleRecord`.

### F. Не проверено мной

Тела обработчиков `bindAutoSave` в `hub-persistence.js` (триггеры сняты по вызовам `scheduleSave`, `:2075-2112`);
поведение `PATCH` при `client_uid` в теле (валидация принимает — переименование через `PATCH` меняет `client_uid`, и
ключ повторного импорта уезжает; проверить, нужно ли); `SampleAnalysisController` на объектном `_turfProfile` (стадия 2
п. 8, как в документе); есть ли у `addSample` в `sample-manager.js` место, где известна площадка помимо ключа стора
(по коду — `_currentSite`; читала выборочно).
