...
(function(global) {
    ...
    // GH-292: _potassiumDisplayLabel/_detectKDisplayRegion/_synthesiseKReconDecision
    // moved to assets/k-reconciliation-decision.js (shared with the Plan
    // page's K Reconciliation preview, which needs them without loading this
    // entire file). Referenced here via window.GAIP_KReconDecision --
    // k-reconciliation-decision.js must load before this file in every blade
    // view that includes it. See that file for full doc comments.
    function _potassiumDisplayLabel(opts) {
        return window.GAIP_KReconDecision.potassiumDisplayLabel(opts);
    }

    function _detectKDisplayRegion(explicit) {
        return window.GAIP_KReconDecision.detectKDisplayRegion(explicit);
    }
    ...
    // file for the full b35fix324/326b doc comment) -- thin wrapper so every
    // existing call site in this file (and word-export-combined.js's
    // _wx._synthesiseKReconDecision(...) calls) keeps working unchanged.
    function _synthesiseKReconDecision(soilData, kRequired, kDelivered, opts) {
        return window.GAIP_KReconDecision.synthesiseDecision(soilData, kRequired, kDelivered, opts);
    }
    ...
    function collectData() {
        ...
        if (window.GAIP_STATE) {
            ...
            if (data.soil.methodology === 'SLAN') {
                ...
                //   3. GH-290 (D07 item 5): HillLabsSampleTypes.deriveCode(species,
                //      texture) — resolves the site's actual certificate match
                //      (S277/S279/S81/S78) from its real species + soil texture,
                //      same resolver mlsnEngine()/SampleAnalysisController.php
                //      already use (GH-260/268). Added because (1) and (2) above
                //      were dead in practice: no caller has ever passed
                //      soilInput.aaSampleType, and `.gaip-aa-sample-type` doesn't
                //      exist in any blade view, so every AA export silently fell
                //      through to the step-4 default below regardless of the
                //      site's real species/texture.
                //   4. Default to S277 (most-common turf code; matches sand-rootzone
                //      perennial ryegrass which dominates the GAIP NZ client base) —
                //      now only reached for genuinely uncovered species/texture
                //      combinations (deriveCode() returns null).
                //
