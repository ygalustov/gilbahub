...
(function(global) {
    ...
    function runDiseaseEngine(state, climateData, shadeResult, waterResult) {
        ...
        // b35fix127: delegate to DiseaseEnginePure (Smith-Kerns + pure models) when
        // available so the shade hub and scenario hub produce identical disease scores.
        // The previous simplified bell-curve model diverged from the scenario hub by
        // 4-10pp because it lacked 5-day rolling humidity, variety modifiers, and
        // DiseaseStressCoupling. Falls back to the legacy model if pure engine not loaded.
        if (global.DiseaseEnginePure && global.GILBA_USE_PURE_DISEASE !== false) {
            try {
                // b35fix129: pull dew and stress from orchestrator computed state rather
                // than building local estimates. On run #1 local estimates use incomplete
                // climate data (no dew engine result yet), producing ~84% instead of 87%.
                // On run #2 the orchestrator has already run and GAIP_DEW_RESULT is set.
                // Using the orchestrator's assembled dewData also ensures the same 5-day
                // rolling leaf-wetness window the scenario hub uses.
                // Similarly, passing real stressAggregates (not null) to coupling ensures
                // the shade hub gets the same coupling result as the scenario hub.
                const _orch = global.GaipOrchestrator;
                // Build inputs in the shape DiseaseEnginePure.analyse() expects.
                // Mirror the orchestrator's buildDiseaseInputs() + buildPureDiseaseInjectables() path.
                const variety = state.turf?.variety;
                const species = state.turf?.grassSpecies || state.turf?.species || '';

                // b35fix129: use orchestrator dew result when available, fall back to estimate
                let dewData = null;
                const orchDew = (_orch && _orch.getComputed('dew')) || global.GAIP_DEW_RESULT || null;
                if (orchDew && orchDew.leafWetness) {
                    dewData = {
                        leafWetness: {
                            totalWetHours: orchDew.leafWetness.totalWetHours || 0,
                            averageWetHours: orchDew.leafWetness.averageWetHours || 0,
                            consecutiveHours: orchDew.leafWetness.consecutiveHours || orchDew.leafWetness.maxConsecutiveWetHours || 0,
                            nightWetHours: orchDew.leafWetness.nightWetHours || 0
                        },
                        dewDuration: orchDew.leafWetness.averageWetHours || 0,
                        dewRisk: orchDew.summary?.diseaseRisk?.conditionsMet?.length > 0 ? 'high'
                               : orchDew.leafWetness.averageWetHours > 8 ? 'moderate' : 'low',
                        source: 'dew-prediction-engine'
                    };
                } else if (climateData?.dewpoint && climateData?.temperature) {
                    const dewpointProximity = (climateData.temperature.min || 15) - (climateData.dewpoint?.mean || 10);
                    let estimatedWetHours = dewpointProximity < 2 ? 6 + (2 - dewpointProximity) * 2
                                         : dewpointProximity < 5 ? 3 : 0;
                    if ((climateData.humidity?.mean || 0) > 85) estimatedWetHours += 2;
                    dewData = {
                        leafWetness: {
                ...
                // b35fix129: use orchestrator stress aggregates for coupling
                const orchStress = (_orch && _orch.getComputed('stress')) || null;

                // Shade inputs — match orchestrator's structural-shade guard
                let shadeInputs = null;
                if (shadeResult) {
                    const svf       = shadeResult.svf       ?? shadeResult.modular?.svf       ?? 1;
                    const facade    = shadeResult.facade     ?? shadeResult.modular?.facade    ?? 0;
                    const treeBlock = shadeResult.treeBlock  ?? shadeResult.modular?.treeBlock ?? 0;
                    const hasStructuralShade = svf < 0.99 || facade > 0 || treeBlock > 0;
                    const deficitPct = hasStructuralShade ? (shadeResult.deficitPct || shadeResult.modular?.deficitPct || 0) : 0;
                    shadeInputs = {
                        stressFactor: hasStructuralShade ? (shadeResult.stressFactor || 0) : 0,
                        dli: shadeResult.DLI_total || shadeResult.dli || null,
                        deficitPct,
                        fungalRisk: shadeResult.fungalRisk || null,
                        dliDeficit: { percentage: deficitPct, mol: hasStructuralShade ? (shadeResult.dliDeficit || 0) : 0 },
                        hasStructuralShade, svf, facade, treeBlock
                    };
                }

                // Regional multipliers
                const regionalMultipliers = {};
                const getMultiplier = global.gaip_getDiseaseMultiplier ||
                    (global.GAIP_VarietyTraits && global.GAIP_VarietyTraits.getDiseaseMultiplier);
                if (typeof getMultiplier === 'function') {
                    ['dollarSpot','brownPatch','pythium','anthracnose','fusarium',
                     'springDeadSpot','helminthosporium','grayLeafSpot','takeAll',
                     'redThread','snowMould','pinkSnowMould','waiteaPatch','largePatch'
                    ].forEach(key => {
                        try { const v = getMultiplier(key); if (typeof v === 'number' && v !== 1) regionalMultipliers[key] = v; }
                        catch (e) { /* ignore */ }
                    });
                }
                ...
                const pureInputs = {
                    ...
                    species,
                    speciesKey: species,
                    effectiveSpeciesKey: species,
                    isOverseed: false,
                    isC4: isC4Species(species),
                    variety: variety && variety !== 'generic' && global.GAIP_VarietyTraits
                        ? (() => { try { return global.GAIP_VarietyTraits.getVarietyData(variety, species); } catch(e) { return null; } })()
                        : null,
                    ...
                    region: (() => {
                        try {
                            ...
                            return (!isNaN(lat) && !isNaN(lon) && typeof global.gaip_detectRegion === 'function')
                                ? global.gaip_detectRegion(lat, lon) : 'au';
                        } catch(e) { return 'au'; }
                    })(),
                    regionalMultipliers,
                    regionDisplayInfo: typeof global.gaip_getRegionDisplayInfo === 'function'
                        ? (() => { try { return global.gaip_getRegionDisplayInfo() || { name: 'Australia' }; } catch(e) { return { name: 'Australia' }; } })()
                        : { name: 'Australia' },
                    largePatchModel: global.LargePatchModel || null,
                    dormancyData: global.GAIP_CLIMATE_V2?.dormancy || null,
                    stressAggregates: orchStress
                };

                const rawResult = global.DiseaseEnginePure.analyse(pureInputs);

                // Apply DiseaseStressCoupling if available (same as orchestrator path)
                let result = rawResult;
                if (global.GAIP_DiseaseStressCoupling && rawResult) {
                    result = global.GAIP_DiseaseStressCoupling.apply(rawResult, orchStress, climateData, { species });
                }

                if (result) {
                    // Map pure engine output to the shape gssh-scenario-engine callers expect
                    const assumptions = ['Smith-Kerns dollar spot model (pure engine)', 'Risk models based on published epidemiological research'];
                    if (state.climate?.useLiveWeather) assumptions.push('Using live weather data');
                    if (variety && variety !== 'generic') assumptions.push(`Variety susceptibility data: ${variety}`);

                    const mappedResult = {
                        // Pure engine .diseases[] uses { displayName, adjustedRisk, riskLevel }
                        // Legacy callers expect { name, risk }
                        diseases: (result.diseases || []).map(d => ({
                            name: d.displayName || d.name,
                            risk: d.adjustedRisk ?? d.riskScore ?? 0,
                            riskLevel: d.riskLevel,
                            optimalTemp: d.optimalTemp || null,
                            primaryDriver: d.primaryDriver || null,
                            source: 'disease-engine-pure'
                        })),
                        overallScore: result.overallScore,
                        overallRisk: result.overallRisk,
                        primaryRisk: result.topThreats?.[0]?.disease || result.diseases?.[0]?.displayName || 'None',
                        topThreats: result.topThreats || [],
                        stressModifier: 1,
                        conditions: {
                            temperature: getAverageTemp(state, climateData),
                            humidity: state.climate?.manual?.moisture?.humidity || climateData?.humidity?.mean || 65,
                            dli: shadeResult?.DLI_total || state.turf?.dli || 20
                        },
                        source: 'disease-engine-pure'
                    };

                    return wrapWithConfidence(mappedResult, {
                        confidence: state.climate?.useLiveWeather && variety && variety !== 'generic' ? 'high' : 'medium',
                        validationLevel: 'validated',
                        sources: ['smith-kerns-2018', 'fidanza-1996'],
                        assumptions,
                        dataQuality
                    });
                }
            } catch (e) {
                console.warn('[GSSH DiseaseEngine] Pure engine failed, falling back to legacy:', e);
            }
        }

        // ── Legacy bell-curve fallback ────────────────────────────────────────
        const assumptions = [
    ...
    // Export to global
    global.GSSH_ScenarioEngine = ScenarioEngine;
    
    // Convenience aliases
    global.gssh_scenario_run = runScenario;
    global.gssh_scenario_compare = compareScenarios;
