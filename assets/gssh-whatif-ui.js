/**
 * GSSH What-If Scenario UI v2.6.5
 * 
 * User interface for A/B scenario comparison
 * Allows users to modify parameters and see impact on turf performance
 * 
 * v2.6.5: Forward-ported from GAIP v2.6.5 (Item 21 merge completion)
 *   - Fixed traffic preset path resolution (traffic.* → inputs.schedule.* alias)
 ...
 *
 * v2.6.4: Forward-ported from GAIP v2.6.4
 *   - Climate fallback chain in adaptStateForEngine — fixes "Missing climate
 ...
 *   - adaptStateForEngine() added to handle hub-orchestrator state format
 *     (inputs.turf, inputs.climate, ...) flattening to engine top-level format
 *   - resolvePresetPath() added to map simple preset paths to actual state paths
 ...
 *   - Now correctly finds compareScenarios/runScenario from GSSH_ScenarioEngine v2.1.0
 ...
 * 
 * @requires gssh-scenario-engine.js
 */
...
(function(global) {
    ...
    const CONFIG = {
        containerClass: 'gssh-whatif-container',
        panelClass: 'gssh-whatif-panel',
        comparableFields: {
            // Water Quality
    ...
    function loadBaseline() {
        // Try GSSH_STATE first (set by hub-tissue during analysis)
        // Note: hub-orchestrator.js may overwrite GSSH_STATE with its own format
        // ({inputs, computed, derived}). If so, adaptStateForEngine() handles it.
        if (global.GSSH_STATE && Object.keys(global.GSSH_STATE).length > 0) {
            currentState.baselineState = JSON.parse(JSON.stringify(global.GSSH_STATE));
            currentState.modifiedState = JSON.parse(JSON.stringify(global.GSSH_STATE));
            
            const format = currentState.baselineState.inputs ? 'orchestrator' : 'tissue-v3';
            console.log('[WhatIfUI] Baseline loaded from GSSH_STATE (' + format + ' format)');
    ...
    function resetToBaseline() {
        ...
        const resultsContainer = document.getElementById('gssh-whatif-results');
        if (resultsContainer) {
    ...
    function renderBaselineSummary(state) {
        const container = document.getElementById('gssh-baseline-summary');
        if (!container) return;
        
        let html = '<div class="gssh-baseline-details" style="font-size: 12px; color: var(--gaip-text);">';
        ...
        if (html === '<div class="gssh-baseline-details" style="font-size: 12px; color: var(--gaip-text);">') {
    ...
    function collectStateFromDOM() {
        ...
        // Water inputs
        const waterEC = document.querySelector('[name="water-ec"], .gssh-water-ec, #water-ec');
        if (waterEC) state.water.ecw = parseFloat(waterEC.value) || 0;
        
        const waterPH = document.querySelector('[name="water-ph"], .gssh-water-ph, #water-ph');
        if (waterPH) state.water.pH = parseFloat(waterPH.value) || 7;
        
        // Turf inputs
        const species = document.querySelector('[name="grass-species"], .gssh-grass-species, #grass-species, select[class*="grass"]');
        if (species) state.turf.grassSpecies = species.value;
        
        const hoc = document.querySelector('[name="hoc"], .gssh-hoc, #hoc, input[class*="hoc"]');
        if (hoc) state.turf.hoc = parseFloat(hoc.value) || 25;
        
        // Traffic inputs
        const matches = document.querySelector('[name="matches"], .gssh-matches, #matches');
        if (matches) state.traffic.matchesPerWeek = parseInt(matches.value) || 0;
        
        const sessions = document.querySelector('[name="sessions"], .gssh-sessions, #sessions');
        if (sessions) state.traffic.sessionsPerWeek = parseInt(sessions.value) || 0;
        
        // Shade inputs
        const svf = document.querySelector('[name="svf"], .gssh-svf, #svf');
    ...
    function applyPreset(presetKey) {
        ...
        for (const [simplePath, value] of Object.entries(preset.changes)) {
            ...
            const inputId = `gssh-param-${simplePath.replace(/\./g, '-')}`;
    ...
    function runComparison() {
        if (!currentState.baselineState || !currentState.modifiedState) {
            ...
            const container = document.getElementById('gssh-whatif-results');
            if (container) {
                container.innerHTML = '<div class="gssh-results-error"><strong>❌ Cannot Compare</strong><p>Please run analysis first to establish baseline state.</p></div>';
            }
            return;
        }
        ...
        const scenarioEngine = global.GSSH_ScenarioEngine ||
        ...
        if (scenarioEngine) {
            try {
                ...
                // v2.4.0: Updated to match GSSH_ScenarioEngine v2.1.0 exports
                if (typeof scenarioEngine.compareScenarios === 'function') {
                    // Primary: GSSH_ScenarioEngine.compareScenarios
                    ...
                    // GSSH_ScenarioEngine.runScenario is the v2.1.0 method
    ...
    function normalizeEngineResult(result) {
        ...
        // Already in expected format? (gssh-scenario-engine output)
        if (result.scenarioA && result.scenarioB && result.comparison) {
            ...
            if (!confidence && result.confidenceComparison) {
                // Transform gssh-scenario-engine confidenceComparison to standard format
                confidence = {
                    comparable: true,
    ...
    function renderBasicComparison() {
        ...
        let html = '<div class="gssh-results-basic" style="padding: 16px;">';
    ...
    function renderPanel() {
        return `
            <div class="${CONFIG.panelClass}">
                ...
                <h3 class="gssh-whatif-title">🔄 What-If Scenario Analysis</h3>
                <p class="gssh-whatif-subtitle">Compare how changes affect turf performance</p>
                
                <div class="gssh-whatif-sections">
                    <div class="gssh-whatif-section gssh-baseline-section">
                        <h4>📍 Current Baseline</h4>
                        <p class="gssh-section-note">Based on your current analysis inputs</p>
                        <div id="gssh-baseline-summary"></div>
                    </div>
                    
                    <div class="gssh-whatif-arrow">→</div>
                    
                    <div class="gssh-whatif-section gssh-scenario-section">
                        <h4>🎯 Scenario B</h4>
                        <p class="gssh-section-note">Modify parameters below or use presets</p>
                        
                        ${renderPresets()}
                        ${renderParameterControls()}
                        
                        <button class="gssh-run-comparison-btn" onclick="GSSH_WhatIfUI.runComparison()">
                            ▶️ Compare Scenarios
                        </button>
                    </div>
                </div>
                
                <div id="gssh-whatif-results" class="gssh-whatif-results">
                    <p class="gssh-results-placeholder">Select a preset or modify parameters, then click "Compare Scenarios"</p>
                </div>
            </div>
        `;
    }
    ...
    function renderPresets() {
        let html = '<div class="gssh-presets">';
        html += '<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">';
        html += '<strong>Quick Presets:</strong>';
        html += '<button class="gssh-reset-btn" onclick="GSSH_WhatIfUI.resetToBaseline()" title="Clear all changes and reset to baseline">🔄 Reset</button>';
        html += '</div>';
        html += '<div class="gssh-preset-grid">';
        
        for (const [key, preset] of Object.entries(PRESETS)) {
            html += `
                <button class="gssh-preset-btn" onclick="GSSH_WhatIfUI.applyPreset('${key}')" title="${preset.description}">
                    ${preset.name}
                </button>
            `;
        }
    ...
    function renderParameterControls() {
        let html = '<div class="gssh-param-controls">';
        html += '<strong>Custom Modifications:</strong>';
        html += '<div class="gssh-param-grid">';
        
        for (const [path, config] of Object.entries(CONFIG.comparableFields)) {
            const inputId = `gssh-param-${path.replace(/\./g, '-')}`;
            
            html += `<div class="gssh-param-item">`;
            html += `<label for="${inputId}">${config.label}</label>`;
            html += `<div class="gssh-param-input-group">`;
            ...
            if (config.type === 'select') {
                ...
                if (config.unit) {
                    html += `<span class="gssh-param-unit">${config.unit}</span>`;
                }
            }
    ...
    function renderConfidenceSection(confidence) {
        ...
        let html = `
            <div class="gssh-confidence-section" style="
                background: ${conf.bg};
    ...
    function renderResults(comparison) {
        ...
        return `
            <div class="gssh-results-grid">
                <!-- Confidence Assessment -->
                ${confidenceHtml}
                
                <!-- Summary Cards -->
                <div class="gssh-results-summary">
                    <div class="gssh-recommendation ${comp.concerns.length === 0 ? 'positive' : comp.improvements.length === 0 ? 'negative' : 'neutral'}">
                        <strong>📋 Recommendation:</strong> ${comp.recommendation}
                    </div>
                </div>
                
                <!-- Delta Table -->
                <div class="gssh-delta-table">
                    ...
                    <table>
                        ...
                        <tbody>
                            <!-- Water Quality -->
                            <tr class="gssh-section-header-row"><td colspan="4">💧 Water Quality</td></tr>
                            ...
                            <!-- Disease -->
                            <tr class="gssh-section-header-row"><td colspan="4">🦠 Disease Risk</td></tr>
                            ...
                            <!-- Traffic/Wear -->
                            <tr class="gssh-section-header-row"><td colspan="4">🏟️ Traffic & Recovery</td></tr>
                            ...
                            <!-- Irrigation -->
                            <tr class="gssh-section-header-row"><td colspan="4">💦 Irrigation</td></tr>
                            ...
                            <tr class="gssh-section-header-row"><td colspan="4">🌱 PGR</td></tr>
                            ...
                            <!-- N Optimization -->
                            <tr class="gssh-section-header-row"><td colspan="4">🧪 Nitrogen</td></tr>
                            ...
                            <!-- Stress Trajectory -->
                            <tr class="gssh-section-header-row"><td colspan="4">📈 14-Day Outlook</td></tr>
                ...
                <div class="gssh-changes-lists">
                    ${comp.improvements.length > 0 ? `
                    <div class="gssh-improvements">
                        <h4>✅ Improvements</h4>
                        <ul>
    ...
    function renderDeltaRow(label, valA, valB, delta, unit, higherIsBetter) {
        ...
        return `
            <tr>
                ...
                <td>${valA}${unit}</td>
                <td>${valB}${unit}</td>
                <td class="gssh-delta ${deltaClass}">${deltaDisplay}</td>
            </tr>
        `;
    }
    ...
    function getStyles() {
        return `
            ...
            .gssh-whatif-subtitle {
                color: var(--gaip-text);
                margin: 0 0 20px 0;
                font-size: 14px;
            }
            
            .gssh-whatif-sections {
                display: grid;
            ...
            .gssh-reset-btn:hover {
                background: rgba(239, 68, 68, 0.4);
                border-color: rgba(239, 68, 68, 0.6);
            }
            
            .gssh-param-controls strong {
                display: block;
                font-size: 12px;
                color: var(--gaip-text);
                margin-bottom: 8px;
            ...
            .gssh-param-item label {
                font-size: 11px;
                color: var(--gaip-text);
            }
            
            .gssh-param-input-group {
                display: flex;
                gap: 4px;
                align-items: center;
            }
            
            .gssh-param-input-group input,
            .gssh-param-input-group select {
                flex: 1;
            ...
            .gssh-param-input-group select option {
            ...
            .gssh-results-error strong {
                color: #fca5a5;
            }
            
            .gssh-results-summary {
                margin-bottom: 16px;
            }
            ...
            .gssh-recommendation.positive {
                background: rgba(34, 197, 94, 0.1);
                border: 1px solid rgba(34, 197, 94, 0.3);
                color: #86efac;
            }
            
            .gssh-recommendation.negative {
                background: rgba(234, 179, 8, 0.1);
                border: 1px solid rgba(234, 179, 8, 0.3);
                color: #fde047;
            }
            
            .gssh-recommendation.neutral {
    ...
    function showWhatIfPanel() {
        ...
        let modal = document.getElementById('gssh-whatif-modal');
        
        if (!modal) {
            // Create modal structure
            modal = document.createElement('div');
            modal.id = 'gssh-whatif-modal';
            modal.style.cssText = `
                position: fixed;
                top: 0;
                left: 0;
            ...
            modalContent.id = 'gssh-whatif-modal-content';
            ...
            panelContainer.id = 'gssh-whatif-container';
    ...
    function hideWhatIfPanel() {
        const modal = document.getElementById('gssh-whatif-modal');
        if (modal) {
