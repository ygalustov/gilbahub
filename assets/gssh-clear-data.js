 ...
 * 
 * Dependencies: sample-manager.js (GSSH_SampleManager)
 */
(function(window, document) {
    ...
    const SELECTORS = {
        // Input containers
        soilGrid:       '.gssh-soil-grid',
        soilPH:         '.gssh-soil-ph',
        soilEC:         '.gssh-soil-ec',
        soilTexture:    '.gssh-soil-texture',
        soilCEC:        '.gssh-cec',
        soilMethodology:'.gssh-soil-methodology',
        samplingDepth:  '.gssh-sampling-depth',
        loi:            '.gssh-loi',
        loiStratified:  '[class*="gssh-loi-"]',

        waterECW:       '.gssh-ecw',
        waterPH:        '.gssh-water-ph',
        waterGrid:      '.gssh-water-grid',
        irrMethod:      '.gssh-irr-method',
        irrEfficiency:  '.gssh-irr-efficiency',
        irrRainEff:     '.gssh-irr-rain-eff',
        irrCost:        '.gssh-irr-cost',
        daysSinceIrr:   '.gssh-days-since-irrigation',
        soilVWC:        '.gssh-soil-vwc',
        rootDepth:      '.gssh-root-depth',
        sensorResult:   '#gssh-sensor-result',
        sensorSummary:  '#gssh-sensor-summary-container',

        tissueModule:   '#gaipTissueModule',

        // Output panels
        soilOutput:     '.gssh-mlsn-body',
        waterOutput:    '.gssh-water-body',
        tissueOutput:   '.gssh-tissue-body',
        nutrientDemand: '.gssh-nutrient-demand-body',

        // Card headers where buttons go
        soilWaterCard:  '.gssh-enable-soil-water',
        tissueCard:     '.gssh-enable-tissue',

        // The h4 section headers inside the soil/water card
        soilMethodLabel:'.gssh-soil-method-label',
        waterHeader:    '.gssh-card-body h4'
    };
    ...
    function flashConfirm(btn, originalLabel) {
        ...
        var hasIcon = btn.querySelector('.gssh-btn-icon');
        
        if (hasIcon) {
            btn.innerHTML = '<span class="gssh-btn-icon">✓</span><span>Cleared</span>';
    ...
    function clearSoilData(btn, domOnly) {
        ...
        document.querySelectorAll(SELECTORS.loiStratified).forEach(function(input) {
            if (input.classList.contains('gssh-loi')) return; // skip main LOI
            input.value = '';
            input.dispatchEvent(new Event('input', { bubbles: true }));
        });
        ...
        if (ndOutput) {
            ndOutput.innerHTML =
                '<p class="gssh-note">Run analysis with soil data to see nutrient demand calculations.</p>';
        }

        // Dispatch event for other modules
        document.dispatchEvent(new CustomEvent('gssh:soil-data-cleared'));
    ...
    function clearWaterData(btn, domOnly) {
        ...
        if (!domOnly && window.GSSH_SampleManager) {
            window.GSSH_SampleManager.clearSamples('water');
            log('clearWater', 'SampleManager water samples cleared');
        }

        // Clear global state
        window.GSSH_WATER_INTERPRETATION = null;
        window.__GSSH_WATER_STATE__ = null;

        // Reset output panel
        var waterOutput = document.querySelector(SELECTORS.waterOutput);
        if (waterOutput) {
            waterOutput.innerHTML =
                '<div class="gssh-water-no-data">' +
                    '<p class="gssh-note">' +
        ...
        // Dispatch event for other modules
        document.dispatchEvent(new CustomEvent('gssh:waterDataCleared'));
    ...
    function clearTissueData(btn) {
        ...
        if (tissueModule) {
            ...
            tissueModule.querySelectorAll('select.gssh-inp').forEach(function(sel) {
                sel.selectedIndex = 0;
            ...
            var ranges = tissueModule.querySelector('.gssh-tissue-ranges');
            if (ranges) ranges.innerHTML = '';
            var bands = tissueModule.querySelector('.gssh-tissue-bands');
            if (bands) bands.innerHTML = '';
        }

        // Clear SampleManager tissue store
        if (window.GSSH_SampleManager) {
            window.GSSH_SampleManager.clearSamples('tissue');
            log('clearTissue', 'SampleManager tissue samples cleared');
        }

        // Clear global state
        window.__GSSH_TISSUE_LAST__ = null;
        window.GSSH_TISSUE_RESULT = null;

        // Reset results output panel
        var tissueOutput = document.querySelector(SELECTORS.tissueOutput);
        if (tissueOutput) {
            tissueOutput.innerHTML =
                '<div class="gssh-tissue-no-data">' +
                    '<p class="gssh-note">' +
        ...
        // Dispatch event for other modules
        document.dispatchEvent(new CustomEvent('gssh:tissueDataCleared'));
    ...
    function injectButtons() {
        ...
        var soilWaterCard = soilWaterCheckbox.closest('.gssh-card');
        ...
        // elements sit outside .gssh-card-body in the parsed DOM.
        var soilMethodLabel = soilWaterCard.querySelector(SELECTORS.soilMethodLabel);
        if (soilMethodLabel && !soilWaterCard.querySelector('.gssh-clear-soil-btn')) {
            ...
            soilBtn.classList.add('gssh-clear-soil-btn');
        ...
        if (waterH4 && !soilWaterCard.querySelector('.gssh-clear-water-btn')) {
            ...
            waterBtn.classList.add('gssh-clear-water-btn');
        ...
        var tissueSwitcher = document.querySelector('.gssh-sample-switcher[data-type="tissue"]');
        if (tissueSwitcher) {
            var importZone = tissueSwitcher.querySelector('.gssh-sample-import-zone');
            if (importZone && !importZone.querySelector('.gssh-clear-tissue-btn')) {
                var tissueBtn = createClearButton('Clear Tissue Data', 'Clear all tissue test inputs and results', clearTissueData);
                tissueBtn.classList.add('gssh-clear-tissue-btn');
                ...
                tissueBtn.innerHTML = '<span class="gssh-btn-icon">🗑️</span><span>Clear Data</span>';
    ...
    document.addEventListener('gssh:site-changed', function(e) {
