<x-email-layout :subject="$isPasswordReset ? 'Reset your Gilba Hub password' : 'Sign in to Gilba Hub'">
  ...
  <h1 style="margin:0 0 10px;font-family:Arial,Helvetica,sans-serif;font-size:22px;font-weight:700;color:#17231f;letter-spacing:-0.4px;line-height:1.2;">
    {{ $isPasswordReset ? 'Reset your password' : 'Sign in to Gilba Hub' }}
  </h1>

  <p style="margin:0 0 30px;font-family:Arial,Helvetica,sans-serif;font-size:15px;color:#5b6a65;line-height:1.65;">
    @if($isPasswordReset)
      Click the button below to reset your Gilba Hub password.
      This link expires in <strong style="color:#17231f;font-weight:600;">15&nbsp;minutes</strong> and can only be used once.
    @else
      Click the button below to sign in to your Gilba Hub account.
      This link expires in <strong style="color:#17231f;font-weight:600;">15&nbsp;minutes</strong> and can only be used once.
    @endif
  </p>
    ...
    <tr>
      <td style="border-radius:8px;background-color:#236b4a;box-shadow:0 1px 3px rgba(35,107,74,0.25);">
        <a href="{{ $url }}"
          ...
          {{ $isPasswordReset ? 'Reset password' : 'Sign in now' }}
        </a>
      </td>
    </tr>
  </table>
